...
Specifically:
* The rasterization algorithm used by the truetype parser in the  [stb项目][stb]may not be able to
...
Let's first consider running ONScripter on Navy native.
Since ONS games generally requires a larger screen to run, we need to define macro `MODE_800x600`
in `navy-apps/libs/libos/src/native.cpp`.
...
Audio: -937182692 Hz 0 bit stereo
```
Where the display resolution is related to the specific ONS game, the output of the audio
information is some garbage, this is because we haven't implemented the sound mixing library
yet. Next ONScripter may trigger an assertion fail in the miniSDL library, or report that it cannot
