...
> rasterize a character from the font library, generating the corresponding pixel information
and a color palette.
> The code adds a small cache when generating the color palette, do you
know how it works?
...
Playing BGM is relatively simple, so we can implement it first. Specifically, you need to implement
the following API:
```c
Mix_Music *Mix_LoadMUS_RW(SDL_RWops *src);
void Mix_FreeMusic(Mix_Music *music);
...
> it is automatically replayed from the beginning, without the user having to play it
manually.
...
> In this way, the operating system can write the virtual address provided by the application directly to the
device.
>
...
> 
> In addition, in order for NEMU to support larger screens, you need to select the screen size
of 800 x 600 in the menuconfig.
> ```
> [*] Devices  --->
>   [*]   Enable VGA  --->
...
> Modern CPUs are usually equipped with a cache, if the latest data that the disk wants to access is in the cache,
then how should DMA access them?
...
As a emasculated OS, Nanos-lite can support quite a few complicated Navy apps such as "仙剑奇侠传".
...
However, There are three programs (NTerm, NSlider和仙剑奇侠传) that are ganrantee to work as long as you accomplish the required assignment.
...
> if there is still an event in the buffer, it will return the event in the buffer, otherwise it will read the event into the buffer
first.
...
> Let Nanos-lite load Fairy Tale and NTerm, then quickly type `F1` and `F2` to switch back and forth between Fairy Tale and NTerm.
...
```
How does "仙剑奇侠传" read pixel information from a file through layers of abstraction in a computer system??
```
...
ramdisk (only when "仙剑奇侠传" archives). Therefore, the sfs we introduced in PA3 works very well
...
which is called "crash consistency" in nature.
...
> The archive architecture in "仙剑奇侠传" `SAVEDGAME_COMMON` is defined in`navy-apps/apps/pal/repo/src/global/global.c`.
...
> You can carefully calculate N according to the definition of the archive structure so that only part
of the data is written to the archive.
> For example, if you archive after a battle and then read the archive right away, you may see that the money is updated, but the character's experience is
not.
>
...
> You don't have to worry about not getting the right
archive back because we can make a new image of the right disk.
...
> For applications inside a virtual
machine, the storage stack to which they write data is actually very deep:
> the data passes through the storage stack of the client machine before reaching the file management module
of the virtual machine software (VMWare or VirtualBox),
...
> From this perspective, forcing a virtual machine to shut down is more likely to cause problems than
forcing a real machine to shut down.
...
> But the most complex problem for a system comes from
the interaction between the various mechanisms:
> what happens when multiple processes
execute `fork()` and then concurrently read and write the same file on disk via `mmap()`?
>
> This question is not intended to be difficult:
> real problems are likely to arise from the
interaction of multiple modules and mechanisms,
...
> Please explain how the paging mechanism and hardware interrupts support the time-sharing of the fairy sword and hello programs on our computer system (Nanos-lite, AM, NEMU) with code examples.
>
...
> }
> ```
> You will see that the program triggers a segmentation error when writing to a read-only
string.
> Please analyze "how the write-protection
mechanism for strings is implemented" from the point of view of the program, compiler, linker, runtime environment, operating system and hardware.
> In other words, what happens
to the computer system that triggers a segmentation error when the above program
executes p[0] = 'A' ?
...
> Then place the lab report file named  `学号.pdf` in the project directory and execute `make submit` to package the project, and finally submit the compressed
package to the specified website.
...
> [PC's Footprints][pc step] series of blog posts provides a brief introduction to the history of
the PC,
...
> so it's good to know the background to the various technologies as well as the context in which
they were developed.
...
Moreover, the birth of the iPhone redefined the concept of the phone, transforming it from a communication tool into a bona fide computer. Thus, [mobile processors][mobile processor] and [mobile operating systems][mobile os] came back into focus. Unlike PCs, mobile processors and operating systems have to tackle the severe challenge of power consumption.
...
As more people, machines, and things connect to the internet, the amount of data generated by these endpoints is increasing exponentially, ushering in the era of [big data][big data]. According to IBM, in 2012, approximately 2.5 exabytes (1EB = 10^6TB) of data were produced daily. To handle this vast amount of data, the concepts of [artificial intelligence (AI)][ai] and [machine learning (ML)][ml] have regained popularity.
However, this brought about a massive data processing challenge, prompting [GPUs][gpu] and [FPGAs][fpga] to join the prolonged battle of big data processing. Another formidable force is the [ASIC][asic], which differs from CPUs in that it is a specialized circuit optimized for a specific application. For instance, AI chips, compared to CPUs, can improve performance-to-power ratios by thousands of times.
The emergence of [AlphaGo][alphago] showcased the immense power of artificial intelligence. Nowadays, AI chips have begun to be integrated into smartphones, making photos more beautiful and applications smarter with use.
...
> #### question::The Monarch of Changes - Computers Revisited
> What is a computer?
> How can seemingly ordinary machinery build such a colorful computer world?
> What do cool game graphics have to do with cold circuits?
>
> ![cs-hierarchy](./images/cs-hierarchy.jpg)
...
> #### flag::The Story of the Birth of the World - Final Chapter
...
>
> This is the end of the story. PA is also coming to an end, but there is no end to the exploration of computers.
> If you want to know what this wonderful world has become, look in the [IA-32 manual][ia-32 manual].
> Or perhaps, with the creativity you've learned from the pioneers, you can change the course of this wonderful world and write a new chapter in the story.
