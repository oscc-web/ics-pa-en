...
Question: Among them, `event` represents the event number, `cause` and `ref` are some supplementary information describing the event,
and `msg` is the event information string. In PA, we will only use `event`.
Then, we only need to define some unified event numbers (the enumeration constants above), and let each architecture describe the reason for the execution flow switch
