...
The heap area is a memory area that is freely used by the program, providing the program with the ability of dynamically allocating memory. The API of TRM only provides the start and end of the heap area, and the allocation and management of the heap area require the program to maintain it by itself. Of course, programs can also not use the heap area, such as `dummy`. Using `putch()` as an API for TRM is a very interesting consideration. We will discuss it soon. For now, we are not planning to run programs that need to call `putch()`.
