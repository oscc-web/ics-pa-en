...
> We can modify the code of `schedule()` to allocate more time slices to The PAL
> so that The PAL is scheduled several times before the hello kernel thread is scheduled once.
...
We will find that after allocating more time slices to The PAL, its running speed has improved to some extent.
...
是阿里巴巴一直都面临的严峻挑战. -->
The scheduling problem in real systems is much more complicated than the example of The PAL.
...
>
> After implementation, let Nanos-lite load the NTerm and hello user processes, and then start the PAL from NTerm.
> If your implementation is correct, you will see the hello user process and NTerm/PAL running in a time-sharing manner.
